...
const MultiRegressionPage: React.FC = () => {
  ...
  const load = async () => {
    try {
      ...
      const { data, error } = await supabase.rpc('get_multi_regression_dataset', {
        target_category: target // Pasamos el argumento a la función
      });
      if (error) throw error;
      
      // La RPC devuelve las filas directamente
      setRows(data);
      // --- FIN DE REEMPLAZO ---
    } catch (e: any) {
      setError(e.message || 'Error al cargar dataset.');
    }
  };
