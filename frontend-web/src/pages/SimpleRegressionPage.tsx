...
// IMPORTANTE: Cambiamos 'api' por 'supabase'
...
// El tipo Row ya coincide con nuestra función RPC
type Row = { month: string; active_students: number; reports: number };

// --- Las funciones de matemáticas no cambian ---
...
// --- Fin de funciones de matemáticas ---
...
const SimpleRegressionPage: React.FC = () => {
  ...
  const load = async () => {
    try {
      setError('');
      // --- LÓGICA REEMPLAZADA (load) ---
      const { data, error } = await supabase.rpc('get_regression_dataset');
  ...
  const handleSave = async () => {
    ...
    try {
      // --- LÓGICA REEMPLAZADA (save) ---
      const { error } = await supabase
        .from('monthly_data') // La nueva tabla
        .upsert({ 
          month: month, 
          active_students: Number(active) 
        }); // 'upsert' inserta o actualiza si el mes ya existe

      if (error) throw error;
      // --- FIN DE REEMPLAZO ---

      setMonth(''); setActive('');
      await load(); // Recargamos los datos
    } catch (e: any) {
